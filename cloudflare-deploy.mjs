 ...
 * Versión ESM compatible con proyectos que tienen "type": "module" en package.json
 */
...
async function main() {
  ...
  if (missingFallbacks.length > 0) {
    ...
    for (const file of missingFallbacks) {
      console.log(`🔧 Creando archivo de respaldo faltante: ${file}`);
      const filePath = path.join(config.fallbackDir, file);
      fs.writeFileSync(filePath, `// Archivo de respaldo generado automáticamente\nconsole.log('[Fallback] Respaldo básico para ${file}');`);
    }
    console.log('✅ Archivos de respaldo creados');
  } else {
