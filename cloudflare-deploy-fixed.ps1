...
if (Test-Path -Path "_routes.json") {
    ...
    Write-Host "Creando _routes.json básico..." -ForegroundColor Yellow
    $routesJson = @"
{
  "version": 1,
  "include": ["/*"],
  ...
  "routes": [
    ...
    {
      "src": "/.*\\.(js|css|png|jpg|gif|svg|ico|json|woff|woff2|ttf|eot)",
      "dest": "/$&"
    },
    {
      "src": "/.*",
...
"@
    Set-Content -Path "dist\_routes.json" -Value $routesJson
}
...
# Preparar archivo para GitHub Actions pero sin utilizar sintaxis que causa problemas en PowerShell
Write-Host "Configurando archivo GitHub Actions para despliegue automático..." -ForegroundColor Yellow
